...
def build_safe_rule_pool(src: bytes, ptrs: tuple[int,...], rule_ids: list[int]):
    ...
    for i,p in enumerate(ptrs):
        ...
        if q:
            blocked.add(q[0])
            reasons.append(('nonrule_master',i,p,q))
    ...
    for o in range(0,len(src)-3,4):
        ...
        if q:
            blocked.add(q[0])
            reasons.append(('other_ptr',o+VA_DELTA,v,q))
