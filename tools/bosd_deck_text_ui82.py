#!/usr/bin/env python3
"""Build the translated DECK/*.DAT text fields while preserving deck composition data.

Only the fixed callout and deck-title fields at the start of each file are changed."""
...
def read_field(b,off,n):
    return b[off:off+n].split(b'\0',1)[0].decode('cp932')
def enc_field(s,n):
    x=s.encode('ascii')
    if len(x)>=n:
        raise ValueError(f'field too long ({len(x)} >= {n}): {s!r}')
...
def main():
    ...
    if set(p.name for p in files)!=set(T):
        missing=sorted(set(T)-set(p.name for p in files))
        extra=sorted(set(p.name for p in files)-set(T))
    ...
    manifest=[]
    for p in files:
        jp1,jp2,en1,en2=T[p.name]
        b=p.read_bytes()
        if len(b)<0x42:
            raise SystemExit(f'{p.name}: too short')
        got1=read_field(b,0x06,40)
        got2=read_field(b,0x2E,20)
        if (got1,got2)!=(jp1,jp2):
            raise SystemExit(f'{p.name}: source gate failed: {(got1,got2)!r}')
        nb=bytearray(b)
        nb[0x06:0x2E]=enc_field(en1,40)
        nb[0x2E:0x42]=enc_field(en2,20)
        out=a.output_dir/p.name
        out.write_bytes(nb)
        if len(nb)!=len(b) or nb[:0x06]!=b[:0x06] or nb[0x42:]!=b[0x42:]:
            raise RuntimeError(f'{p.name}: containment failed')
        if read_field(nb,0x06,40)!=en1 or read_field(nb,0x2E,20)!=en2:
            raise RuntimeError(f'{p.name}: output decode failed')
