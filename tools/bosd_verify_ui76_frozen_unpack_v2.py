...
def outer(b):
    ...
    if decl != len(b):
        raise SystemExit(f'ERROR: declared size {decl} != {len(b)}')
...
def main():
    ...
    C=a.compiled.read_bytes()
    if len(R)!=SIZE or sha(R)!=RETAIL_SHA:
        raise SystemExit('ERROR: retail UNPACK preflight failed')
    if len(C)!=SIZE:
        raise SystemExit(f'ERROR: compiled UNPACK size {len(C)} != {SIZE}')
    ro,co=outer(R),outer(C)
    if ro!=co:
        raise SystemExit('ERROR: outer SDA offsets changed')
    rows=list(csv.DictReader(a.inventory.open(encoding='utf-8-sig',newline='')))
    if len(rows)!=677:
        raise SystemExit(f'ERROR: inventory rows={len(rows)}')
    exp=set(int(r[k]) for r in rows for k in ('large_chunk','small_chunk'))
    if len(exp)!=1354:
        raise SystemExit('ERROR: expected card chunk set malformed')
    changed=set()
