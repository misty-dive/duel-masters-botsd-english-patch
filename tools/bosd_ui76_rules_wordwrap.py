...
def main():
    ...
    a=ap.parse_args()
    mf,mrows=read_rows(a.master_in)
    df,drows=read_rows(a.display_in)
    rules={}
    inserted=0
    changed=0
    max_before=0
    ...
    if len(rules)!=654:
        raise ValueError(f'expected 654 rules, got {len(rules)}')

    display_rule_rows=0
    for r in drows:
        mid=(r.get('master_text_id') or '').strip()
        if mid and int(mid) in rules:
            r['translation']=rules[int(mid)]
            display_rule_rows+=1
    write_rows(a.master_out,mf,mrows)
    write_rows(a.display_out,df,drows)
    ...
    for mid in range(len(chk)):
        ...
        if isrule and cout[mid]['translation']!=rules[mid]:
            raise ValueError(f'roundtrip mismatch rule {mid}')
