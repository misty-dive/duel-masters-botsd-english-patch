...
# This changes only the verified keyboard default-state bytes in the executable.
MODE_VA = 6456619
VA_DELTA = 1044480
...
def main():
    ...
    if src[MODE_OFF] != OLD:
        raise SystemExit(f'ERROR: initial keyboard mode byte at {MODE_OFF:#x} is {src[MODE_OFF]}, expected {OLD}')
    out = bytearray(src)
    out[MODE_OFF] = NEW
    blob = bytes(out)
    ...
    lines = ['UI81 KEYBOARD DEFAULT-ENGLISH PATCH', f'input_sha256={got}', f'output_sha256={sha(blob)}', f'file_size={len(blob)}', f'mode_virtual_address={MODE_VA:#x}', f'mode_file_offset={MODE_OFF:#x}', f'initial_mode={OLD}->6 (A/ABC lowercase page)', 'patch_bytes=1', 'keyboard_pages=UNCHANGED', 'keyboard_mode_switching_code=UNCHANGED', 'RESULT=PASS']
    text = '\n'.join(lines) + '\n'
    print(text, end='')
    if a.report:
