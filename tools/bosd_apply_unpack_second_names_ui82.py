...
# Only the second 677-card sprite layer is replaced; all other UNPACK chunks are containment-checked.
SIZE = 151525376
FIRST = 2037
LAST = 2713
...
def bounds(offs, i, total):
    return (offs[i], offs[i + 1] if i + 1 < len(offs) else total)

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('ui76_compiled', type=Path)
    ...
    if len(P) != SIZE:
        raise SystemExit(f'ERROR: input size {len(P)} != {SIZE}')
    offs = outer(P)
    with zipfile.ZipFile(a.patch_zip) as z:
        rows = list(csv.DictReader(io.TextIOWrapper(z.open('manifest.csv'), encoding='utf-8')))
        if len(rows) != 677:
            raise SystemExit(f'ERROR: patch manifest rows={len(rows)}')
        out = bytearray(P)
        touched = []
        for r in rows:
            idx = int(r['chunk'])
            if idx < FIRST or idx > LAST:
                raise SystemExit(f'ERROR: bad patch chunk {idx}')
            s, e = bounds(offs, idx, len(P))
            src = P[s:e]
            if sha(src) != r['retail_sha256']:
                raise SystemExit(f'ERROR: chunk {idx} input is not exact retail preimage')
            q = z.read(f'chunks/{idx:04d}.bin')
            if len(q) != e - s or sha(q) != r['patched_sha256']:
                raise SystemExit(f'ERROR: patch payload failed at chunk {idx}')
            out[s:e] = q
            touched.append(idx)
    if touched != list(range(FIRST, LAST + 1)):
        raise SystemExit('ERROR: target chunk sequence incomplete')
    a.output.parent.mkdir(parents=True, exist_ok=True)
    a.output.write_bytes(out)
    Q = a.output.read_bytes()
    for i in range(len(offs)):
        s, e = bounds(offs, i, len(P))
        diff = P[s:e] != Q[s:e]
        if diff != (FIRST <= i <= LAST):
            raise SystemExit(f'ERROR: archive containment mismatch chunk {i}, diff={diff}')
